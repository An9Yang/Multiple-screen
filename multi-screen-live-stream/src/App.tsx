...
function App() {
  ...
  const [windowPositions, setWindowPositions] = useState<Record<number, WindowPosition>>({
    1: { x: 100, y: 100, width: 480, height: 320, zIndex: 1 }
  });
    ...
    if (activeGames.length < 4) {
      ...
      setWindowPositions({
        ...
        [game.id]: {
          ...
          width: 480,
          height: 320,
          zIndex: Math.max(...Object.values(windowPositions).map(p => p.zIndex), 0) + 1
        }
      });
    }
  };
  ...
  const bringToFront = (gameId: number) => {
    const maxZ = Math.max(...Object.values(windowPositions).map(p => p.zIndex));
      ...
      <header className="bg-gray-800 border-b border-gray-700 relative z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl font-bold text-white">Soccer Live TV</h1>
          </div>
